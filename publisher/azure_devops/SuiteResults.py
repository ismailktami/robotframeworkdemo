...
class SuiteResults(ResultVisitor):
        ...
        for s in suite.suites:
            if len(s.tests)>0:
                for tc in s.tests:
                    infoTestCase = {}
                    try:
                            infoTestCase['id'] = SuiteResults.get_tags_with_prefixe(tc,'ID-')
                            ...
                            infoTestCase["planId"]=SuiteResults.get_tags_with_prefixe(tc,'TESTPLAN-')
                            infoTestCase["suiteId"]=SuiteResults.get_tags_with_prefixe(tc,'SUITE-')
                            tests_ids_status[infoTestCase['id']]=infoTestCase['status']
                            testcases[infoTestCase['id']]=infoTestCase
                            suiteId=SuiteResults.get_tags_with_prefixe(tc,'SUITE-')
                            testplanId=SuiteResults.get_tags_with_prefixe(tc,'TESTPLAN-')
                    except Exception as ex:
                            tests_witout_ids.append({"name":tc.name})
                            pass
                logger.warning("\nSutie "+s.name+" ==> Tests without IDs:\n"+str(tests_witout_ids))
                self.suites.append({'name':s.name,'id':suiteId,'planId':testplanId,'tests_ids_status':tests_ids_status,'tests_witout_ids':tests_witout_ids,'testcases':testcases,'starttime':suite.starttime,'endtime':suite.endtime})
    ...
    def get_tags_with_prefixe(testcase,tag_perfixe):
        try:
            if testcase.tags :
                id=[id for id in testcase.tags if str(id).startswith(tag_perfixe)][0].split(tag_perfixe)[1]
                return id
        except Exception:
            raise Exception("Tag with prefix "+tag_perfixe+" dosent exist in test case "+testcase.name)
